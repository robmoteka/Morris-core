...
function initBootstrapTabs() {
  const tabEls = document.querySelectorAll(
    '#stepsTab button[data-bs-toggle="tab"]'
  );
...
function initJsonEditors() {
  ...
  jsonEditorContainers.forEach((container) => {
    ...
    const inputField = document.querySelector(
      `.step-params-input[name="steps[${index}][params]"]`
    );
...
function setupAddStepTab() {
  ...
  if (addStepTab) {
    addStepTab.addEventListener("click", function () {
      ...
      const stepTabs = document.querySelectorAll(
        ".steps-tabs .nav-item:not(:last-child)"
      );
...
function createStepContent(index) {
  ...
  content.id = `step-content-${index}`;
  content.role = "tabpanel";
  content.setAttribute("aria-labelledby", `step-tab-${index}`);
  ...
  content.innerHTML = `
    <div class="step-item">
      ...
      <!-- Wybór typu kroku -->
      <div class="mb-3">
        <label for="stepType-${index}" class="form-label">Typ kroku</label>
        <select class="form-select step-type-select" id="stepType-${index}" name="steps[${index}][type]" required>
          <option value="processor">Processor - przetwarza dane wejściowe</option>
          <option value="transformer">Transformer - transformuje dane</option>
          <option value="validator">Validator - sprawdza dane</option>
          <option value="filter">Filter - filtruje dane</option>
          <option value="plugin">Plugin - używa wtyczki</option>
        </select>
      </div>

      <!-- Sekcja wyboru pluginu (widoczna tylko dla typu "plugin") -->
      <div class="plugin-selection-section mb-3 d-none">
        <div class="mb-3">
          <label class="form-label">Rodzaj pluginu</label>
          <select class="form-select plugin-type-select" name="steps[${index}][plugin_type]">
            <option value="local">Plugin lokalny</option>
            <option value="remote">Plugin zewnętrzny</option>
          </select>
        </div>
        
        <!-- Lista lokalnych pluginów -->
        <div class="local-plugin-selector mb-3">
          <label class="form-label">Plugin lokalny</label>
          <select class="form-select plugin-select" name="steps[${index}][plugin]">
            ${pluginOptions}
          </select>
          <div class="form-text plugin-description mt-2"></div>
        </div>
        
        <!-- Pole dla zdalnych pluginów -->
        <div class="remote-plugin-field mb-3 d-none">
      ...
      <!-- Konfiguracja kroku -->
      <div class="mb-3">
        <label class="form-label">Konfiguracja</label>
...
function initJsonEditorForStep(index) {
  const container = document.querySelector(
    `.json-editor[data-index="${index}"]`
  );
  const inputField = document.querySelector(
    `.step-params-input[name="steps[${index}][params]"]`
  );
  const typeSelect = document.querySelector(`#stepType-${index}`);
  const pluginSelectionSection = document.querySelector(
    `#step-content-${index} .plugin-selection-section`
  );
  const pluginTypeSelect = document.querySelector(
    `#step-content-${index} .plugin-type-select`
  );
  const localPluginSelector = document.querySelector(
    `#step-content-${index} .local-plugin-selector`
  );
  const remotePluginField = document.querySelector(
    `#step-content-${index} .remote-plugin-field`
  );
  const pluginDescription = document.querySelector(
    `#step-content-${index} .plugin-description`
  );
  const pluginSelect = document.querySelector(
    `#step-content-${index} .plugin-select`
  );
  ...
  // Ustawienie domyślnych konfiguracji dla różnych typów kroków
  const defaultConfig = {
    processor: { method: "transform" },
    transformer: { mapping: { output: "$.input" } },
    validator: { schema: { type: "object" } },
    filter: { condition: "$.value > 10" },
    plugin: {},
  };

  // Ustawienie domyślnej konfiguracji
  const selectedType = typeSelect.value;
  const defaultConfigForType = defaultConfig[selectedType] || {};
  editor.set(defaultConfigForType);
  inputField.value = JSON.stringify(defaultConfigForType, null, 2);

  // Zapisz referencję do edytora w atrybucie data
  container.editor = editor;

  // Funkcja aktualizująca widoczność sekcji pluginów
  function updatePluginVisibility() {
    if (typeSelect.value === "plugin") {
      pluginSelectionSection.classList.remove("d-none");

      // Aktualizuj widoczność w zależności od typu pluginu
      const pluginType = pluginTypeSelect.value;
      if (pluginType === "local") {
        localPluginSelector.classList.remove("d-none");
        remotePluginField.classList.add("d-none");
      } else {
        localPluginSelector.classList.add("d-none");
        remotePluginField.classList.remove("d-none");
      }
    } else {
      pluginSelectionSection.classList.add("d-none");
    }
  }

  // Obsługa zmiany typu kroku
  typeSelect.addEventListener("change", function () {
    const newConfig = defaultConfig[this.value] || {};
    editor.set(newConfig);
    inputField.value = JSON.stringify(newConfig, null, 2);

    // Aktualizuj widoczność sekcji pluginów
    updatePluginVisibility();
  });

  // Obsługa zmiany typu pluginu (lokalny/zewnętrzny)
  if (pluginTypeSelect) {
    pluginTypeSelect.addEventListener("change", updatePluginVisibility);
  }

  // Obsługa wyboru pluginu z listy
  if (pluginSelect) {
    pluginSelect.addEventListener("change", function () {
      ...
      if (pluginDescription) {
        pluginDescription.textContent = description;
      }

      // Jeśli wybrano "Inny zdalny plugin", przełącz na widok pluginu zewnętrznego
      if (this.value === "remote:") {
        pluginTypeSelect.value = "remote";
        updatePluginVisibility();
      }
    });
  }

  // Inicjalizuj widoczność na początku
  updatePluginVisibility();
}

/**
 * Konfiguruje obsługę usuwania kroków
 */
    ...
    newButton.addEventListener("click", function (e) {
      ...
      if (isActive) {
        const firstTab = document.querySelector(
          ".steps-tabs .nav-link:not(.add-step-tab)"
        );
        if (firstTab) {
...
function setupPluginSelects() {
  ...
  pluginSelects.forEach((select) => {
    ...
    newSelect.addEventListener("change", function () {
      ...
      const descriptionContainer = stepItem.querySelector(
        ...
        if (remoteInput) {
          ...
          newInput.addEventListener("input", function () {
            ...
            if (remotePluginName) {
              ...
              const pluginSelect =
                this.closest(".step-item").querySelector(".plugin-select");
              if (pluginSelect) {
      ...
      if (selectedOption.dataset.paramsDescription) {
        paramsHelpContainer.innerHTML =
          selectedOption.dataset.paramsDescription;
      } else {
...
function updateFormFromJson() {
  try {
      ...
      jsonData.steps.forEach((step, index) => {
        ...
        setTimeout(() => {
          ...
          if (stepContent) {
            ...
            const remoteField = stepContent.querySelector(
              ".remote-plugin-field"
            );
            const remoteInput = stepContent.querySelector(
              ".remote-plugin-name"
            );
            ...
            if (pluginSelect) {
              ...
              if (pluginId.startsWith("remote:")) {
                ...
                const remoteOption = Array.from(pluginSelect.options).find(
                  (opt) => opt.value === "remote:"
                );
                if (remoteOption) {
    ...
    alert(
      "Wystąpił błąd podczas aktualizacji formularza. Sprawdź poprawność JSON."
    );
  }
}
