...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('morris.log')
    ]
)
...
def start():
    ...
    try:
        ...
        if not czy_proces_dziala(proces.pid):
            logger.error("Aplikacja zakończyła działanie tuż po uruchomieniu. Sprawdź logi.")
            return False
    ...
    if czy_proces_dziala(appPid):
        try:
            ...
            if czy_proces_dziala(appPid):
                ...
                logger.warning(f"Proces główny nadal działał, wysłano SIGKILL (PID: {appPid})")
    ...
    if czy_proces_dziala(mqttPid):
        try:
            ...
            if czy_proces_dziala(mqttPid):
                ...
                logger.warning(f"Proces MQTT nadal działał, wysłano SIGKILL (PID: {mqttPid})")
...
def status():
    ...
    statusInfo = {
        ...
        "port": {
            "zajety": czy_port_zajety(APP_PORT),
            "numer": APP_PORT
        }
    }
...
def wyswietl_status():
    ...
    print(f"Główny proces: {'DZIAŁA' if statusInfo['aplikacja']['dziala'] else 'NIE DZIAŁA'}")
