...
class ChainEngine:
    ...
    def __init__(self, mqtt_client=None, chains_file="chains/chains.json"):
        """
    ...
    def load_chains(self):
        ...
        if not os.path.exists(self.chains_file):
            logger.warning(f"Plik z definicjami chainów nie istnieje: {self.chains_file}")
            return
        ...
        try:
            ...
            logger.info(f"Załadowano {len(self.chains)} chainów z pliku {self.chains_file}")
    ...
    def _validate_chain(self, chain_definition):
        ...
        if 'steps' not in chain_definition or not isinstance(chain_definition['steps'], list):
    ...
    def _save_chains(self):
        ...
        try:
            ...
            logger.info(f"Zapisano {len(self.chains)} chainów do pliku {self.chains_file}")
        ...
        for step_index, step in enumerate(chain.get('steps', [])):
            ...
            try:
                ...
                if ':' in plugin_name:
                    ...
                    current_data = self._run_remote_plugin(plugin_name, current_data, plugin_config)
                else:
                    # Plugin lokalny
                    current_data = self._run_local_plugin(plugin_name, current_data, plugin_config)
            
            except Exception as e:
                logger.error(f"Błąd podczas wykonywania kroku {step_index+1} (plugin '{plugin_name}'): {e}")
    ...
    def run_chain_async(self, trigger_id, payload, callback=None):
        ...
        logger.info(f"Uruchomiono asynchroniczne przetwarzanie chaina dla triggera '{trigger_id}'")
    ...
    def _run_remote_plugin(self, plugin_name, data, config):
        ...
        try:
            ...
            logger.error(f"Błąd podczas uruchamiania zdalnego pluginu '{plugin_name}': {e}")
            return data
    ...
    def _handle_plugin_response(self, msg):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Otrzymana odpowiedź nie jest poprawnym JSON: {msg.payload}")
                return
            
            logger.debug(f"Otrzymano odpowiedź od zdalnej wtyczki {device_id}: {payload}")
            ...
            logger.error(f"Błąd podczas przetwarzania odpowiedzi od zdalnej wtyczki: {e}")
    ...
    def _setup_mqtt_callbacks(self):
        ...
        if not hasattr(self.mqtt_client, 'client') or self.mqtt_client.client is None:
            logger.warning("Klient MQTT nie jest jeszcze zainicjalizowany - callbacki zostaną skonfigurowane później")
            return
    ...
    def _process_remote_plugin(self, plugin_name, data, params=None, timeout=5):
        ...
        """
        if not self.mqtt_client:
            logger.error(f"Brak klienta MQTT. Nie można wywołać zdalnej wtyczki {plugin_name}.")
            return data
        ...
        try:
            ...
            logger.warning(f"Timeout podczas oczekiwania na odpowiedź od zdalnej wtyczki {plugin_name}")
            ...
            logger.error(f"Błąd podczas przetwarzania przez zdalną wtyczkę {plugin_name}: {e}")
