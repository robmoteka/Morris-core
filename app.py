...
def send_test():
    ...
    test_data = {
        ...
        "data": {
            "message": "To jest testowa wiadomość z Morris Core",
            "value": 42
        }
    }
...
def manage_chains():
    ...
    if request.method == 'GET':
        ...
        if success:
            return jsonify({
                "status": "success",
                "message": f"Dodano chain: {chain_id}"
            })
        else:
            return jsonify({
                "status": "error",
                "message": "Nie udało się dodać chaina"
            }), 500
...
def manage_chain(chain_id):
    ...
    if request.method == 'GET':
        if chain_id in chain_engine.chains:
            ...
            return jsonify({
                "status": "error",
                "message": f"Chain {chain_id} nie istnieje"
            }), 404
        ...
        if 'trigger' not in chain_definition or 'steps' not in chain_definition:
            return jsonify({
                "status": "error",
                "message": "Wymagane pola: trigger, steps"
            }), 400
        ...
        if success:
            return jsonify({
                "status": "success",
                "message": f"Zaktualizowano chain: {chain_id}"
            })
        else:
            return jsonify({
                "status": "error",
                "message": "Nie udało się zaktualizować chaina"
            }), 500
        ...
        if success:
            return jsonify({
                "status": "success",
                "message": f"Usunięto chain: {chain_id}"
            })
        else:
            return jsonify({
                "status": "error",
